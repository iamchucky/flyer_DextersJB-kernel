...
static void touchEventReport(struct ntg_tsData *ntg)
{
	if (ntg->touchEvnt->header.type == 0x01
		...
		if (event_google_enable == 1) {
			compatible_tp_report(ntg->input_dev, &ntg->touchEvnt->stouch, 0x00, true, (ntg->touchEvnt->stouch.bit & 0x02));
		...
		if (event_google_enable == 1) {
		  if ( ntg->touchEvnt->ptouch.bit == 3 || ntg->touchEvnt->ptouch.bit == 7 || ntg->touchEvnt->ptouch.bit == 13 ) {
			...
			input_sync(ntg->input_devP);
                        ...
                        input_report_abs(ntg->input_devP, ABS_MT_TOUCH_MAJOR, 0x00);
                        input_report_key(ntg->input_devP, BTN_TOUCH, 0);
                        input_report_key(ntg->input_devP, BTN_TOOL_PEN, 0);
                        input_report_abs(ntg->input_devP, ABS_MT_TRACKING_ID, 0);
                        input_report_abs(ntg->input_devP, ABS_MT_TOUCH_MAJOR, 0);
                        input_report_abs(ntg->input_devP, ABS_MT_WIDTH_MAJOR, 0);
                        input_report_abs(ntg->input_devP, ABS_MT_POSITION_X, 0);
                        input_report_abs(ntg->input_devP, ABS_MT_POSITION_Y, 0);
                        input_report_abs(ntg->input_devP, ABS_MT_PRESSURE, 0);
			input_sync(ntg->input_devP);
		  	}
		  input_sync(ntg->input_devP);
		} else {
...
static int ntrig_ts_probe(struct spi_device *dev)
{
	...
	input_set_abs_params(ntg_ts->input_devP, ABS_MT_POSITION_X, ntg_ts->abs_x_min, ntg_ts->abs_x_max, 0, 0);
	input_set_abs_params(ntg_ts->input_devP, ABS_MT_POSITION_Y, ntg_ts->abs_y_min, ntg_ts->abs_y_max, 0, 0);
	input_set_abs_params(ntg_ts->input_devP, ABS_MT_TOUCH_MAJOR, ntg_ts->abs_pressure_min, ntg_ts->abs_pressure_max, 0, 0);
	input_set_abs_params(ntg_ts->input_devP, ABS_MT_WIDTH_MAJOR, ntg_ts->abs_width_min, ntg_ts->abs_width_max, 0, 0);
...
/*#ifndef CONFIG_TOUCHSCREEN_COMPATIBLE_REPORT*/
	...
	input_set_abs_params(ntg_ts->input_devP, ABS_MT_AMPLITUDE, 0, ((255 << 16) | 32), 0, 0);
	input_set_abs_params(ntg_ts->input_devP, ABS_MT_POSITION, 0, ((ntg_ts->abs_x_max << 16) | ntg_ts->abs_y_max), 0, 0);
/*#endif*/
